...
## Setting Gate Findings
- 当前仓库已有三块可复用底盘：
  - `strong_quality_gate` 的 truth-source / sync pollution 规则
  - `sync-setting-assets.py` 的设定沉淀写盘逻辑
  - `project-maintenance.py` 的写后维护链
- 真正缺失的不是单个规则，而是统一的前置 gate：
  - `novel-init` 只会建极薄 starter files
  - `novel-outline` 能完成，但不会强制收设定覆盖度
  - `novel-write` 会读设定，但不会先查一个统一 gate 状态
- 两个真实新项目 `projects/公司裁我那天，系统先赔了我一百万` 与 `projects/转学第一天，我把校草认成了新来的代课老师` 都验证了这个缺口：
  - 已写到第 3 章
  - `设定集/` 仍主要只有 `主角.md` 与 `力量体系.md`
- 已新增：
  - `docs/setting-gate-policy.json`
  - `scripts/setting_gate.py`
  - `tests/test_setting_gate.py`
- `setting_gate.py` 当前第一版已经具备：
  - 读取项目 state / 大纲 / 现有设定集
  - 本地优先自动补角色 / 地点 / 势力 / 物品骨架卡
  - 用 `strong_quality_gate.detect_missing_truth_sources` 做高风险 truth gap 阻断
  - 产出 `.mighty/setting-gate.json`
  - 产出带 `source_stage / confidence / requires_user_confirmation` 的 `sync-review.json`
- `project-maintenance.py` 已改成先跑 `setting_gate.py --stage write-post`，再继续 sync / guidance split / thin-state
- `sync-setting-assets.py` 的 review queue 现在会保留已有 gate 队列项并补齐扩展字段，而不是简单覆盖
- skill / workflow 合同已完成接线：
  - `novel-init`：显式写入 `setting gate(init)`
  - `novel-outline`：显式写入 `setting gate(outline)` 硬门
  - `novel-write`：显式要求先检查 `.mighty/setting-gate.json`
  - `novel-sync`：显式声明 `sync-review` 是共享队列
  - `novel-scan`：显式声明只给 `setting gate` 提供 candidate，不直接写 canon
- 对两个稀疏真实项目的 `--report-only` smoke 结果显示：
  - `公司裁我那天，系统先赔了我一百万` 会自动补出角色 / 地点 / 物品卡
  - `转学第一天，我把校草认成了新来的代课老师` 会自动补出角色 / 地点 / 物品卡
  - 当前这两条线都未触发高风险 truth gap，因此 `status = passed`
