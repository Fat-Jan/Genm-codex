...
>
> 这是当前正式长期架构收口台账与重开入口；文件名继续保留 `architecture-open-issues.md` 以兼容既有链接。根目录 `架构问题跟踪.md` 只做提醒入口，不替代本文件。

## 目的

这份文档记录 `Genm-codex` 已正式确认并完成首轮收口、但仍值得跨会话回看的架构问题与重开条件。
...
- 当前若没有 `open / planned / monitoring / deferred` 条目，本文件承担“已收口台账”角色；若后续出现新问题，可直接在本文件重开
...
- 当前 AOI-001 ~ AOI-008 均已标记为 `resolved`
- 本文件当前不表示存在新的架构阻塞项
- 当前角色是“已收口架构台账 + 重开条件索引”，而不是“仍有未决问题待排期”的 active 问题池
