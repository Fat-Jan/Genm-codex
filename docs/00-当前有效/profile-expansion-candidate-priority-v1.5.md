...
1. 当前样本是否已经有真实承载。
2. 当前 bucket / profile / positioning 是否存在明显缺口。
...
这一组已经有真实样本承载，且最适合优先补 `platform_positioning` / overlay / profile 收口。
...
这一组不是不能补，而是应等前两批把 contract、positioning 和 overlay 读取链跑顺后再动。
...
- 它们很适合做 profile 扩面，但不适合作为第一批 registry 收口对象
