...
- `active-context` 只保留当前 prompt 装配需要的摘要 / 指针
- 不复制 `.mighty/learned-patterns.json` 里的 `recent_guardrails` 正文
- 真要读 guardrails 细节，应回到 `.mighty/learned-patterns.json`
...
- 记录主 profile / 主 bucket
- 记录 `tagpacks / strong_tags / narrative_modes / tone_guardrails`
- 给 `outline / write / review / package` 输出压缩消费结果
