...
如果后续要从当前治理模式继续往 selective-sync 演进，最稳的第一步是：

1. 保持 governance-aware full-copy 仍可用
2. 先支持按域同步：
   - `profiles`
   - `references`
   - `templates`
...
这里的 governance-aware full-copy 指：

- 先 `--report` / `--report-json`
- 默认阻断 `same-path drift overwrite`
- sync 后恢复 `shared/sync-governance.json` 中声明的 protected local paths
