# Writing Core Smoke Draft: 她升职那天，前上司成了我合租室友

> 当前结果为 `writeback`，需人工确认。

## 适用范围

- 项目：`projects/她升职那天，前上司成了我合租室友`
- bucket：`职场婚恋`
- 章节范围：`001-003`

## 证据来源

- `projects/她升职那天，前上司成了我合租室友/.mighty/state.json`
- `projects/她升职那天，前上司成了我合租室友/.mighty/learned-patterns.json`
- `projects/她升职那天，前上司成了我合租室友/.mighty/market-adjustments.json`
- `projects/她升职那天，前上司成了我合租室友/大纲/总纲.md`
- `projects/她升职那天，前上司成了我合租室友/chapters/第001章.md`
- `projects/她升职那天，前上司成了我合租室友/chapters/第002章.md`
- `projects/她升职那天，前上司成了我合租室友/chapters/第003章.md`

## 包装判断样本

```md
packaging_judgment:
  current_title: keep
  opening_method_cue: 第一章先把升职接锅和合租揭面双困局砸下来，第二章误判升级，第三章拿下试运行窗口。
  genre_difference_cue: 职场婚恋要让工作链路和同屋边界同场推进，不能只写旧情和空糖。
  premium_review_cue: 后续章节要把试运行窗口兑现成更实的数据、续约和抢功代价，不要让关系推进脱离业务。
  overpromise_warning: 不要提前包装成男女主已经结盟或项目已经稳了，也不要把前上司写成会直接开后门的安全男主。
  payoff_timing_reminder: 第一轮兑现应落在72小时试运行窗口；下一轮尽快落到续约结果、公司内抢功和同住越界成本。
  title: 她升职那天，前上司成了我合租室友
  one_liner: 姜栀升职当天接下一个随时会砸掉她职位的大客户，回到新租房却发现新室友正是刚跳去甲方的前上司裴述。
  synopsis: 姜栀升职当天，接下的是一个随时会砸掉她职位的大客户；回到新租的房子，她发现新室友竟是刚跳去甲方的前上司裴述。一个掌握续约生死，一个连退房都退不起，他们只能在工作链路冲突和同屋檐下的生活摩擦里，一边算项目账，一边算关系账。
```

## 预检判断样本

```md
precheck_summary:
  投稿建议: ready-now
  packaging-needs-update: no
  content_standard_summary:
    primary_failure_mode: none
    execution_risk: low
    packaging_alignment_note: 不要提前包装成男女主已经结盟或项目已经稳了，也不要把前上司写成会直接开后门的安全男主。
```

## writeback 预览

```md
writeback_preview:
  chapter: 003
  fields: ['content_standard_flags', 'packaging_alignment_note']
  bucket: 职场婚恋
  packaging_status: written
  writeback_status: written
```

## 收口更新

- 已生成：
  - `包装/包装方案.md`
- `packaging-needs-update` 已被消化成实际包装产物。
