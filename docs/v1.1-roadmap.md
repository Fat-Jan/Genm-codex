# Genm-codex `v1.1` 路线草案

## 目的

这份文档回答：

- 如果 `v1.0.0` 之后要继续推进，最值得做的 `v1.1` 方向是什么

它不是发布承诺，而是当前最合理的增强路线草案。

---

## 路线判断

`v1.1` 不应该回到“大量扩新命令”的思路。

最合理的增强方向仍然围绕已经验证过价值的主线：

1. 包装层
2. 质量路由
3. 学习回写

并且其中最值得优先切入的新方向，不是“再加一个包装命令”，而是：

- **番茄短故事 / 中短故事适配**
- **番茄长篇优先桶的数据驱动重排**

它更像包装层之前的上游约束层。

补充：

- `setting gate` 的 triage / recovery 收口已经应当按 `v1.0.x` 默认主线稳定化处理
- 不应把这类“让默认工作流更可恢复”的工作重新包装成 `v1.1` 新主线

---

## 候选主线

### 1. 包装层增强

优先级：高

目标：

- 让 `novel-package` 在真实项目里更稳定
- 减少包装与正文承载的偏差

候选改进：

- 包装层与已有 review / precheck 信号更强联动
- 更稳定的“主方案 / 备选 / 暂不推荐”结构
- 更清晰的包装更新条件
- 接入番茄短故事分类与内容桶约束，而不是只做长篇主站包装

#### 番茄短故事适配子线

建议顺序：

1. 先做短故事分类研究
2. 再做短故事约束建模
3. 再让包装层消费这些约束

而不是反过来。

参考：

- [fanqie-short-story-adaptation.md](/Users/arm/Desktop/vscode/Genm-codex/docs/fanqie-short-story-adaptation.md)
- [fanqie-content-buckets.md](/Users/arm/Desktop/vscode/Genm-codex/docs/fanqie-content-buckets.md)
- [fanqie-bucket-constraints.md](/Users/arm/Desktop/vscode/Genm-codex/docs/fanqie-bucket-constraints.md)
- [fanqie-paid-signal-matrix.md](/Users/arm/Desktop/vscode/Genm-codex/docs/fanqie-paid-signal-matrix.md)

当前第一批实际接入点：

- `novel-package`
- `novel-precheck`
- `novel-outline`

也就是：

- 先让大纲层、包装层和预检层吃到 `content_bucket` 与桶级约束
- 再考虑是否继续往 `genre / write` 深接
- 长篇优先级不再只按“热榜感觉”排，而是按：
  - 热度
  - 收益倾向
  - 平台扶持
  - 系统适配
  四维打分来排

### 2. 质量路由增强

优先级：高

目标：

- 继续收紧 `review / fix / polish / rewrite / precheck` 的口径

候选改进：

- 降低 `review` 与 `precheck` 的口径差异
- 更稳定的 `recommended_next_action`
- 更明确的批量投稿判断

### 3. 学习回写增强

优先级：中高

目标：

- 让 `learned_patterns` 对后续正文写作的作用更实用

候选改进：

- 让 `write / polish / precheck` 更显式消费 learn 信号
- 减少 learn 结果的泛化和空泛描述

### 4. `state` 瘦身与设定同步层

优先级：中高

目标：

- 把 `.mighty/state.json` 收成运行态
- 补上 `state -> 设定集` 的同步层

候选改进：

- 明确哪些字段继续留在 `state`
- 避免角色 / 地点 / 势力继续主要堆在 `state`
- 增加最小的 `setting-sync` 或等价流程

参考：

- [state-thinning-and-setting-sync.md](/Users/arm/Desktop/vscode/Genm-codex/docs/state-thinning-and-setting-sync.md)

---

## 暂缓项

### `novel-scan` 正式化

继续暂缓，不建议作为 `v1.1` 主线。

原因：

- 它仍然是实验能力
- 当前不阻塞默认主线
- 它适合单独作为实验能力演进线

### shared 同步策略切换

继续暂缓。

原因：

- 工程价值高，但不直接提升默认写作主线
- 更适合在明确需要时再处理

---

## 当前推荐

如果后续继续推进 `v1.1`，最合理的优先顺序是：

1. **番茄优先内容池研究**
2. **番茄内容桶约束建模**
3. **番茄付费信号矩阵与 MVP 桶选择**
4. **包装层增强**
5. 质量路由增强
6. 学习回写增强

也就是说，真正顺序不是：

- 先做包装
- 再想平台

而应该是：

- 先做番茄内容池与分类桶
- 先确认哪些桶更接近“可赚钱”
- 再让包装层消费这些约束

当前额外结论：

- **女频付费线已进入第一梯队 MVP**
- 不是等男频线全部做完再考虑
- 当前更合理的第一批方向是：
  - 长篇：`宫斗宅斗` / `传统玄幻` / `奇幻仙侠` / `玄幻脑洞`
  - 短篇：`现实情感` / `现言甜宠` / `悬疑脑洞`
- 当前第一批执行顺序明确为：
  1. `现实情感`
  2. `宫斗宅斗`
  3. `传统玄幻`
- 在第一批桶稳定后，优先叠加的标签包顺序为：
  1. `恶女 x 宫斗宅斗`
  2. `恶女 x 现实情感`
  3. `恶女 x 现言甜宠`
  4. `恶毒女配 x 双女主替身白月光`
- 当前已冻结的第一条内部生产模板为：
  - 暂无
- 当前已收口出的首个可投样本为：
  - 暂无
- 当前待重验的高优先级返工样本为：
  - `恶女 x 宫斗宅斗`
  - 参考：
    - [fanqie-evil-gongdou-production-template.md](/Users/arm/Desktop/vscode/Genm-codex/docs/research/fanqie/fanqie-evil-gongdou-production-template.md)
    - [fanqie-evil-gongdou-submission-assessment.md](/Users/arm/Desktop/vscode/Genm-codex/docs/research/fanqie/fanqie-evil-gongdou-submission-assessment.md)
- 当前已收口出的第二条生产模板候选为：
  - `恶女 x 现实情感`
  - 参考：
    - [fanqie-evil-qinggan-production-candidate.md](/Users/arm/Desktop/vscode/Genm-codex/docs/research/fanqie/fanqie-evil-qinggan-production-candidate.md)
- 当前第三条线仍保持：
  - `恶女 x 现言甜宠`
  - 定位：实验线
  - 参考：
    - [fanqie-evil-tianchong-experiment-status.md](/Users/arm/Desktop/vscode/Genm-codex/docs/research/fanqie/fanqie-evil-tianchong-experiment-status.md)
- 当前第四条线新增为：
  - `恶毒女配 x 双女主替身白月光`
  - 定位：第四条付费候选实验线
  - 参考：
    - [fanqie-evil-dual-female-substitute-candidate.md](/Users/arm/Desktop/vscode/Genm-codex/docs/research/fanqie/fanqie-evil-dual-female-substitute-candidate.md)

参考：

- [fanqie-first-execution-plan.md](/Users/arm/Desktop/vscode/Genm-codex/docs/fanqie-first-execution-plan.md)
- [fanqie-mvp-bucket-templates.md](/Users/arm/Desktop/vscode/Genm-codex/docs/fanqie-mvp-bucket-templates.md)
- [fanqie-mvp-tagpacks.yaml](/Users/arm/Desktop/vscode/Genm-codex/docs/fanqie-mvp-tagpacks.yaml)

---

## 一句话结论

`v1.1` 最该做的，不是“更多命令”，而是：

- **把已经证明有价值的默认主线做得更稳、更一致。**
